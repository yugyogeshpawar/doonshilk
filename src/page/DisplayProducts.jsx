...
import { Card, CardBody, CardHeader, Container, Form, Input } from 'reactstrap'
...
function EditSaveButton({ row, onEdit }) {
  const [showDialog, setShowDialog] = useState(false);

  return (
    <>
      <Btn color="warning" onClick={() => { setShowDialog(true) }} id={row.ID}>
...

      {
        (
          showDialog &&
          <EditProductForm data={row} onClose={() => {
            setShowDialog(false);
            onEdit && onEdit();
            // alert('close');
          }} />
...
export default function DisplayProducts() {
  ...
  const tableColumns = ((handleDelete) => [
    ...
    {
      name: 'Edit / Save',
      cell: (row) => <EditSaveButton row={row} onEdit={(edit) => {
