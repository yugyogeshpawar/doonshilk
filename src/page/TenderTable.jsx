...
function UpdateModal(props) {
  ...
  const update = () => {
    ...
    axios.post(`${baseUrl}updateAnnouncementForm`, { is_active: props.updateName, id: props.updateId, text: props.updateText })
      .then(
        (response) => {
      ...
      <ModalBody >
        <FormGroup>
          ...
          </select>
          {
            props.updateText &&
            (
              <Input type="textarea" rows="3" value={props.updateText} onChange={(e) => { props.setUpdateText(e.target.value) }} required />
            )
          }
        </FormGroup>
...
function TenderTable() {
  ...
  const handleUpdate = (row) => {
    ...
    setOpen(true)
    setUpdateId(row.id)
    setUpdateText(row.text ? row.text : null);
    setReload(!reload)
